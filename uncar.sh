...
check_sum ()
{
    ...
    file_md5=$( $MD5SUM -b "${file}" | cut -d ' ' -f1 )
    
    if [[ $sum == ${file_md5} ]] ; then 
	echo 0
    else
	echo 1
    fi
...
main ()
{
	...
	if [[ ${CAR_DATA["name"]} ]] ; then 
		...
		_check=$(check_sum ${CAR_DATA["name"]} ${CAR_DATA["md5sum"]} )

		if [ $_check -eq 0 ] ; then echo "checksum OK" ; else echo "checksum FAIL" ; fi
	    fi
